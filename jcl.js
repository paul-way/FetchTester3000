...
(function() {
    ...
    JCL.RetrieveEntityRequest = function(logicalName, fnCallBack) {
        ...
        return JCL._ExecuteRequest(request, "Execute", JCL._RetrieveEntityResponse, fnCallBack);
    };

    JCL._RetrieveEntityResponse = function(xmlhttp, callback) {
    ...
    /*		//resultDoc.async = false;
		...
		for (i = 0; i < entList.length; i++) {
			...
			for (j = 0; j < attrCount; j++) {
				...
				if (sBase === "Attributes") {
					curr = entList[i].childNodes[j].childNodes;
					entityRef.Attributes = {};

					for (l = 0; l < curr.length; l++) {

						if (curr[l][xmlLabel] === "AttributeMetadata") {
							attr = {};
							for (m = 0; m < curr[l].childNodes.length; m++) {
								singleAttr = {};
								singleAttr.push(curr[l].childNodes[m][xmlLabel]);
								singleAttr.push(curr[l].childNodes[m][xmlValue]);
								attr.push(singleAttr);
							}
							entityRef.Attributes.push(attr);
						}
					}
				}				
			}
